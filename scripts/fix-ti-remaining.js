...
console.log('=== Fixing Remaining TI Issues ===\n');
    ...
    if (product.alternatives && product.alternatives.parts) {
      product.alternatives.parts.forEach(alt => {
        ...
        if (!alt.comparison || !alt.comparison.includes('=><')) {
          alt.comparison = `${product.partNumber}=><${alt.partNumber}: 输入电压范围相似，输出电流能力相当，封装形式兼容，可直接替换使用`;
        } else {
          // Add voltage/current specs if missing
          if (!alt.comparison.includes('电压') && !alt.comparison.includes('电流')) {
            alt.comparison = alt.comparison.replace(/[=><]+/, '=><') + '，输入电压范围和输出电流能力相当';
          }
        }
      });
    }
...
console.log('✓ Fixed products.json alternativeParts format');

// Fix solutions.json - faeInsights with complete structure
...
solutionsData.solutions.forEach(solution => {
  // Fix faeInsights with complete structure including decisionFramework
...
// Fix support.json - faeInsights with complete structure
